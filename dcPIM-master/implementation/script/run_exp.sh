...
for addr in  "${ssh_array[@]: 1 : $num_hosts}";
	do 
	 	# TODO: Update SSH user, key path, domain, and which hosts are senders.
...
# TODO: Update this to the host that should run in "start" mode.
...
for addr in  "${ssh_array[@]: 0 : $num_hosts}";
	do 
	 	# TODO: Update SSH user, key path, domain for your servers.
